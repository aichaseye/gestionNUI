...
export class DiplomeService {
  ...
  protected convertDateFromClient(diplome: IDiplome): IDiplome {
    return Object.assign({}, diplome, {
      anneeObtention: diplome.anneeObtention?.isValid() ? diplome.anneeObtention.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.anneeObtention = res.body.anneeObtention ? dayjs(res.body.anneeObtention) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((diplome: IDiplome) => {
        diplome.anneeObtention = diplome.anneeObtention ? dayjs(diplome.anneeObtention) : undefined;
      });
    }
    return res;
  }
