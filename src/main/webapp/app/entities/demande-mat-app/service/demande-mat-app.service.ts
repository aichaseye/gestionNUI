...
export class DemandeMatAppService {
  ...
  protected convertDateFromClient(demandeMatApp: IDemandeMatApp): IDemandeMatApp {
    return Object.assign({}, demandeMatApp, {
      dateNaissance: demandeMatApp.dateNaissance?.isValid() ? demandeMatApp.dateNaissance.format(DATE_FORMAT) : undefined,
      anneeDemande: demandeMatApp.anneeDemande?.isValid() ? demandeMatApp.anneeDemande.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.anneeDemande = res.body.anneeDemande ? dayjs(res.body.anneeDemande) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((demandeMatApp: IDemandeMatApp) => {
        demandeMatApp.dateNaissance = demandeMatApp.dateNaissance ? dayjs(demandeMatApp.dateNaissance) : undefined;
        demandeMatApp.anneeDemande = demandeMatApp.anneeDemande ? dayjs(demandeMatApp.anneeDemande) : undefined;
      });
    }
    return res;
  }
