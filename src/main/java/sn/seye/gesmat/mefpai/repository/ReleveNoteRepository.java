...
public interface ReleveNoteRepository extends JpaRepository<ReleveNote, Long> {
    ...
    @Query(
        value = "select distinct releveNote from ReleveNote releveNote left join fetch releveNote.apprenant left join fetch releveNote.filiereEtude left join fetch releveNote.serieEtude",
    ...
    @Query(
        "select distinct releveNote from ReleveNote releveNote left join fetch releveNote.apprenant left join fetch releveNote.filiereEtude left join fetch releveNote.serieEtude"
    )
    List<ReleveNote> findAllWithToOneRelationships();

    @Query(
        "select releveNote from ReleveNote releveNote left join fetch releveNote.apprenant left join fetch releveNote.filiereEtude left join fetch releveNote.serieEtude where releveNote.id =:id"
    )
