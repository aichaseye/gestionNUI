...
public interface PersonnelRepository extends JpaRepository<Personnel, Long> {
    ...
    @Query(
        value = "select distinct personnel from Personnel personnel left join fetch personnel.etablissement left join fetch personnel.inspection",
    ...
    @Query(
        "select distinct personnel from Personnel personnel left join fetch personnel.etablissement left join fetch personnel.inspection"
    )
    List<Personnel> findAllWithToOneRelationships();

    @Query(
        "select personnel from Personnel personnel left join fetch personnel.etablissement left join fetch personnel.inspection where personnel.id =:id"
    )
