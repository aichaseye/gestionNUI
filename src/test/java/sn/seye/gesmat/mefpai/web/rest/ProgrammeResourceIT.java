...
import sn.seye.gesmat.mefpai.repository.ProgrammeRepository;
import sn.seye.gesmat.mefpai.service.ProgrammeService;
...
class ProgrammeResourceIT {
    ...
    public static Programme createEntity(EntityManager em) {
        Programme programme = new Programme().nomModule(DEFAULT_NOM_MODULE).coef(DEFAULT_COEF).note(DEFAULT_NOTE);
        return programme;
    }
    ...
    public static Programme createUpdatedEntity(EntityManager em) {
        Programme programme = new Programme().nomModule(UPDATED_NOM_MODULE).coef(UPDATED_COEF).note(UPDATED_NOTE);
        return programme;
    }
    ...
    void createProgramme() throws Exception {
        ...
        assertThat(testProgramme.getNomModule()).isEqualTo(DEFAULT_NOM_MODULE);
        assertThat(testProgramme.getCoef()).isEqualTo(DEFAULT_COEF);
        assertThat(testProgramme.getNote()).isEqualTo(DEFAULT_NOTE);
    }
    ...
    void getAllProgrammesWithEagerRelationshipsIsEnabled() throws Exception {
        when(programmeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restProgrammeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(programmeServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllProgrammesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(programmeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getProgramme() throws Exception {
        ...
        restProgrammeMockMvc
            ...
            .andExpect(jsonPath("$.nomModule").value(DEFAULT_NOM_MODULE))
    ...
    void putNewProgramme() throws Exception {
        ...
        updatedProgramme.nomModule(UPDATED_NOM_MODULE).coef(UPDATED_COEF).note(UPDATED_NOTE);
        ...
        Programme testProgramme = programmeList.get(programmeList.size() - 1);
        assertThat(testProgramme.getNomModule()).isEqualTo(UPDATED_NOM_MODULE);
        assertThat(testProgramme.getCoef()).isEqualTo(UPDATED_COEF);
        assertThat(testProgramme.getNote()).isEqualTo(UPDATED_NOTE);
    }
    ...
    void partialUpdateProgrammeWithPatch() throws Exception {
        ...
        partialUpdatedProgramme.nomModule(UPDATED_NOM_MODULE).coef(UPDATED_COEF);
    ...
    void fullUpdateProgrammeWithPatch() throws Exception {
        ...
        partialUpdatedProgramme.nomModule(UPDATED_NOM_MODULE).coef(UPDATED_COEF).note(UPDATED_NOTE);
